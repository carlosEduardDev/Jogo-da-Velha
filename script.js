"use strict";
const markupFields = Array.from(document.querySelectorAll(".game button"));
...
let heldPositions = [];
let currentPlayer = "X";
const addEvents = (fields) => {
    fields.forEach((btn) => {
        btn.addEventListener("click", turnPlayer);
    });
};
function turnPlayer({ currentTarget }) {
    currentPlayer === "X" ? (currentPlayer = "O") : (currentPlayer = "X");
    if (currentTarget instanceof HTMLButtonElement && turn && imgTurn) {
        turn.innerHTML = `Vez do Jogador: <img src="${currentPlayer === "X"
            ? (imgTurn.src = "../assets/o.png")
            : (imgTurn.src = "../assets/x.png")}" alt="player"/>`;
...
function finishGame() {
    if (turn &&
        ...
        turn.innerHTML = `O jogador <img src="${currentPlayer === "X"
            ? (imgTurn.src = "../assets/x.png")
            : (imgTurn.src = "../assets/o.png")}" alt="player"/> venceu!`;
        currentPlayer === "X"
...
function tie() {
    if (heldPositions.filter((item) => item).length === 9) {
